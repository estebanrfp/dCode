...
// public relays unless DCODE_RELAY names one of your own (a local relay makes
// discovery deterministic; the public ones can take a while to pair peers).
const ROOT = new URL("..", import.meta.url).pathname.replace(/\/$/, "")
const TYPES = { html: "text/html", js: "text/javascript", css: "text/css", json: "application/json", svg: "image/svg+xml" }
Bun.serve({
  port: Number(process.env.PORT) || 5805,
  async fetch(req) {
...
console.log(`dCode on http://localhost:${Number(process.env.PORT) || 5805}`)
