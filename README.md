...
- `tests/agent.spec.js` — the agent as a peer of its own, with the model replaced by a stub that streams a fixed file: the desk window signs in as the agent while the owner stays herself, the branch it owns named after the brief, the file landing line by line with the model's fences left out, the commit under the agent's signature, the pull request the other visitor holds over the wire, and the owner's fast-forward merge that leaves `main` running the app.
