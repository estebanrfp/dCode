...
Optional, and a helper that proposes. The prompt box sits at the bottom right of every repository page. Describe an application and the agent writes it as one complete HTML file over GenosDB, line by line into the shared buffer of a branch it owns, named after your brief — everyone in the room watches it appear — then commits and opens a pull request into the branch you stood on. You merge it, or keep editing its branch. Ask again and it changes the file, not starts it over: its branch begins as the file you stood on, and what the model dictates is fitted onto it as it arrives — a line the file already holds keeps its node, a changed one is rewritten in place, only what is new is inserted and only what is gone is removed.

**It is a peer of its own.** The box opens the agent's desk: a small window of dCode signed in as the agent, an identity minted once on this computer and kept there, that takes your brief over the room's channel. Its commits carry its signature and its address, its branch is its own, and it can no more move your `main` than any other visitor — the pull request is a proposal until you merge it, with your signature. The cronology reads who wrote what.

**Nothing leaves the machine.** The model runs in the desk's window on WebGPU: `Qwen2.5-Coder`, 7B by default, 3B and 1.5B when the GPU is smaller (`?model=<id>` to choose), downloaded once from the model hub and cached by the browser. What it knows is in [agent-prompt.md](agent-prompt.md) — the rules of a single-file GenosDB application, the exact API it may use and a complete example to follow — plus GenosDB's own [summary for models](https://cdn.jsdelivr.net/gh/estebanrfp/gdb@main/llms.txt), fetched fresh on each first use so the agent follows the engine; and whatever the model does with the rules, the skeleton is enforced as the file lands: a module script, the CDN import, the room named after the brief. Edit the prompt file to change how it builds. It needs a browser with WebGPU: Chrome, Edge or Safari 26.
...
Playwright, one `BrowserContext` per simulated visitor (own storage, own identity), a fresh room per test, real WebRTC between them. Twelve tests in seven files:
...
- `tests/agent.spec.js` — the agent as a peer of its own, with the model replaced by a stub that streams a fixed file: the desk window signs in as the agent while the owner stays herself, the branch it owns named after the brief, the file landing line by line with the model's fences left out, the commit under the agent's signature, the pull request the other visitor holds over the wire, the owner's fast-forward merge that leaves `main` running the app, and a second brief fitted onto the file: every seeded node kept, the changed line rewritten in place, one node for the one line added.
