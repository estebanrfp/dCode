...
- **branch** — a name and a `head`. The owner grants `write` to collaborators with `db.sm.acls.grant`; anyone else who commits gets a branch of their own, forked from where they stood, with a copy of the buffer — and the buffer they edited rejoins its head, so the owner finds their branch as they left it. Two writers moving one head at once: the hybrid logical clock keeps one, the other commit stays in the graph, behind, and merges like any other. Its owner can delete it — asked twice — except the repository's default one: the buffer's lines go, the commits stay in the timeline.
...
Playwright, one `BrowserContext` per simulated visitor (own storage, own identity), a fresh room per test, real WebRTC between them. Twenty-six tests in ten files:
...
- `tests/branches.spec.js` — a fork that takes the buffer with it and leaves main at its head, a pull request, a proposal brought up to date and a fast-forward merge that the target's buffer follows; a tampered client that writes another's branch head and is refused by every receiver, proved against a later write that lands; a collaborator granted `write` who moves the head directly, and is back to forking once revoked; the owner granting `write` at the very instant a collaborator moves the head — two writes over one branch node, from two devices — and both surviving on every peer, the newcomer then moving the head too: the engine keeps one write and re-applies the other's field over it. A branch you own deleted, asked twice, its lines gone on both peers and the owner back on main; main and other people's branches offer no such button.
