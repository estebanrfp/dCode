...
- `tests/agent.spec.js` — the agent as your own hands, with the model replaced by a stub that streams a fixed file: the template becoming the app in the buffer under Alice's session, on Bob's screen too, the commit message prefilled and the commit hers; a second brief fitted onto the file with every node kept, the heading rewritten in place and one node for the added line; and Bob asking on Alice's repository — the shared buffer changing for everyone, his commit a fork of his own, the pull request from him.
