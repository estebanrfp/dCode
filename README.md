...
Optional, and a helper: a coding model that runs in your browser, on WebGPU, and proposes. The prompt box sits at the bottom right of every repository page. Describe an application and the model writes it as one complete HTML file over GenosDB, line by line into the shared buffer of a branch named after your brief — everyone in the room watches it appear — then commits and opens a pull request into the branch you stood on. You merge it, or keep editing its branch. It never moves `main`.

Nothing leaves the machine: the weights (`Qwen2.5-Coder`, 7B by default, 3B and 1.5B when the GPU is smaller; `?model=<id>` to choose) are downloaded once from the model hub and cached by the browser, and the prompt is not sent anywhere. What the model knows is in [agent-prompt.md](agent-prompt.md) — the rules of a single-file GenosDB application and the exact API it may use — plus GenosDB's own [summary for models](https://cdn.jsdelivr.net/gh/estebanrfp/gdb@main/llms.txt), fetched fresh on each first use so the agent follows the engine. Edit the prompt file to change how it builds. It needs a browser with WebGPU: Chrome, Edge or Safari 26.
...
- `tests/agent.spec.js` — the agent's flow with the model replaced by a stub that streams a fixed file: a branch of its own named after the brief, the file landing line by line with the model's fences left out, the commit, the pull request the other visitor holds over the wire, and the owner's fast-forward merge that leaves `main` running the app.
