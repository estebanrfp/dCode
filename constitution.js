...
export const CONSTITUTION = {
  ...
  principles: [
    ...
    ["A name is a label you sign", "What you call yourself is a field of your own `user:` node — the one the engine ties to your key. Only you can write it: every peer recovers the signature and refuses anyone else's, and the same gate refuses your own write if it tries to touch your role. Two identities may choose the same name; the address underneath is the one that cannot be copied."],
    ["Presence is signed, and decides nothing", "The dot that says who is inside a repository is an announcement the window signs with its key: every peer recovers who sent it and puts that name on the dot, and on the caret beside the code, which rides unsigned behind it. Nothing is written; a window that closes takes its dot with it; a visitor with no session is counted and named nobody; a forged or stale announcement is dropped. The graph never reads any of it — presence labels, it never decides."],
