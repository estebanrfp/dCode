...
function afterChange() {
  clearTimeout(changeTimer); changeTimer = setTimeout(() => view?.renderRepoBar(), 120)
...
document.addEventListener("cut", (e) => {
  if (readOnlyView()) return
  if (allSelected) { e.preventDefault(); e.clipboardData.setData("text/plain", domText()); setAllSelected(false); view.applyText(current.branch, ""); return }
...
document.addEventListener("paste", (e) => {
  ...
  if (allSelected) { e.preventDefault(); setAllSelected(false); view.applyText(current.branch, text); return }
  if (range) { e.preventDefault(); replaceRange(rangeLines(), text) }
})
document.addEventListener("keydown", (e) => {
  if (!allSelected || readOnlyView()) return
  if (e.key === "Backspace" || e.key === "Delete") { e.preventDefault(); setAllSelected(false); view.applyText(current.branch, "") }
...
const loadView = () => (viewLoading ??= import("@repo").then((m) => { view = m; bringAgent(); render() },
...
document.addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (act === "revoke-member") {
      ...
      view.forgetMembers(); view.renderMembers(repo); return
    }
    if (act === "edit-repo") {
    ...
    if (act === "discard") { const b = currentBranch(); if (!b) return; pendingMerge.delete(b.id); await flushSaves(); await view.applyText(b.id, contentOf(b.value.head)); return }
    if (act === "load-commit") { const b = currentBranch(), c = commitOf(a.dataset.commit); if (!b || !c) return; await flushSaves(); await view.applyText(b.id, contentOf(c.id)); toast(`${short(c.id)} is now the buffer of ${branchLabel(nodes.get(b.value.repo), b)}. Commit it to make it the head again.`); return }
    if (act === "merge") { e.preventDefault(); const pr = nodes.get(a.dataset.pr); if (pr) await view.mergePR(pr); return }
...
document.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (f.id === "member-form") { // an envelope on the vault: the engine wraps the key for an address that has signed in once
      ...
      f.reset(); toast(`${nameOf(address)} holds the key now; their page opens on its own.`, "success"); view.forgetMembers(); view.renderMembers(repo); return
    }
...
const bringAgent = async () => (await import("@agent")).mountAgent({ currentBranch, me: () => me, flushSaves, bufferLines: () => [...buffer().children].map((li) => ({ id: li.id, text: fieldOf(li).value, order: orderOf(li) })), putLine, keysBetween, remove: (id) => db.remove(id), commit: async (message) => { // the buffer repaints a frame after the writes: press Commit once it shows them
